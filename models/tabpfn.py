...
from models.tabpfn_utils import normalize_data, to_ranking_low_mem, remove_outliers
from models.tabpfn_utils import NOP, normalize_by_used_features_f
...
from models.tabpfn_utils import load_model_only_inference
import os
import pickle
import io
...
class CustomUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        if name == 'Manager':
            from settings import Manager # TODO: `pip install python-settings` if needed
            return Manager
        try:
...
def load_model_workflow(i, e, add_name, base_path, device='cpu', eval_addition='', only_inference=True):
    ...
    if only_inference:
        ...
        raise RuntimeError("Are you trying to train the model? Uncomment line 103 and modify the code then")
        model, c = None, None
...
class TabPFNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y, overwrite_warning=False):
        ...
        if self.no_grad:
            ...
            X, y = check_X_y(to_numpy(X), to_numpy(y), force_all_finite=False)
    ...
    def predict_proba(self, X, normalize_with_test=False, return_logits=False):
        ...
        pred = None
        while k < n:
            right = min(k + max_batch_size, n)
            X = X0[k:right]
            if self.no_grad:
                X = check_array(to_numpy(X), force_all_finite=False)
            ...
            k = right
            pred = prediction_ if pred is None else torch.cat([pred, prediction_])
            # print('pred.shape =', pred.shape)

        return pred.detach().cpu() if self.no_grad else pred
