"""
    Code adapted from RTDL's (gorishniy2021revisiting) codebase
"""
...
from models.rtdl_resnet import make_nn_module, GEGLU, ReGLU

_INTERNAL_ERROR_MESSAGE = "RTDL paper's problem"
...
class Transformer(nn.Module):
    ...
    def __init__(
            ...
            *,
            ...
            head_activation: Union[str, Callable[..., nn.Module]],
            head_normalization: Union[str, Callable[..., nn.Module]],
            d_out: int,
    ) -> None:
        super().__init__()
        ...
        assert all_or_none([n_tokens, kv_compression_ratio, kv_compression_sharing]), (
        ...
        for layer_idx in range(n_blocks):
            ...
            if layer_idx or not prenormalization or first_prenormalization:
                layer['attention_normalization'] = make_nn_module(
                    attention_normalization, d_token
                )
            layer['ffn_normalization'] = make_nn_module(ffn_normalization, d_token)
...
class FTTransformer(nn.Module):
    ...
    # TODO: not used in Tabzilla, I checked this function returns the same parameters as .parameters()
    # TODO: the only difference is weight_decay is only applied to one set of parameters, it's okay to use .parameters()
    ...
    # TODO: not used in Tabzilla, it's okay to use our optimizer
...
def _is_glu_activation(activation: Union[str, Callable[..., nn.Module]]):
    return (
