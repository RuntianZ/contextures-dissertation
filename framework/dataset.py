...
class TabularDataset(Dataset):
    def __init__(
        ...
        self.get_id = False
        self.pred_transform = []
        self.pred_proba_transform = []

    def register_pred_transform(self, func):
        self.pred_transform = [func] + self.pred_transform

    def register_pred_proba_transform(self, func):
        self.pred_proba_transform = [func] + self.pred_proba_transform

    def get_pred_for_eval(self):
        y = self.pred
        for func in self.pred_transform:
            y = func(y)
        return y

    def get_pred_proba_for_eval(self):
        y = self.pred_proba
        for func in self.pred_proba_transform:
            y = func(y) 
        return y 
